...
class PythonDefaultTaintChecker extends PythonTaintAbstractChecker {
  ...
  prepareEntryPoints(analyzer, dir, moduleManager, fileManager) {
    ...
    const { entrypoints: ruleConfigEntryPoints } =
      this.checkerRuleConfigContent;
    ...
    if (config.entryPointMode !== "ONLY_CUSTOM") {
      ...
      if (pythonDefaultRule[0].checkerIds.includes(this.getCheckerId())) {
        ...
        this.checkerRuleConfigContent.sources.TaintSource = Array.isArray(
          ...
          ? this.checkerRuleConfigContent.sources.TaintSource
          : [this.checkerRuleConfigContent.sources.TaintSource];
        this.checkerRuleConfigContent.sources.TaintSource.push(
          ...pythonDefaultRule[0].sources.TaintSource
        );
      }
      const { pyFcEntryPointArray, pyFcEntryPointSourceArray } =
        findPythonFcEntryPointAndSource(dir, fileManager);
      ...
      if (pyFcEntryPointSourceArray) {
        ...
        this.checkerRuleConfigContent.sources.TaintSource = Array.isArray(
          ...
          : [this.checkerRuleConfigContent.sources.TaintSource];
        this.checkerRuleConfigContent.sources.TaintSource.push(
          ...pyFcEntryPointSourceArray
        );
      }
    }

    if (
      config.entryPointMode !== "SELF_COLLECT" &&
      !_.isEmpty(ruleConfigEntryPoints)
    ) {
    ...
    for (const funCallEntryPoint of funCallEntryPoints) {
      let valFuncs = AstUtil.satisfy(
        ...
        (n) =>
          n.vtype === "fclos" &&
          extractRelativePath(n?.ast?.loc?.sourcefile, dir) ===
            funCallEntryPoint.filePath &&
    ...
    for (const fileEntryPoint of fileEntryPoints) {
      const fullFilePath =
        `${config.maindir}${fileEntryPoint.filePath}`.replace("//", "/");
      ...
      if (file?.ast?.type === "CompileUnit") {
        ...
        entryPoint.filePath =
          file?.ast?.sourcefile || file?.ast?.loc?.sourcefile;
  ...
  triggerAtIdentifier(analyzer, scope, node, state, info) {
    IntroduceTaint.introduceTaintAtIdentifier(
      node,
      info.res,
  ...
  triggerAtFunctionCallBefore(analyzer, scope, node, state, info) {
    ...
    IntroduceTaint.introduceFuncArgTaintByRuleConfig(
      fclos?.object,
      node,
      argvalues,
  ...
  triggerAtFunctionCallAfter(analyzer, scope, node, state, info) {
    ...
    IntroduceTaint.introduceTaintAtFuncCallReturnValue(
      fclos,
      node,
      ret,
  ...
  checkByNameMatch(node, fclos, argvalues) {
    ...
    if (rule) {
      ...
      const ndResultWithMatchedSanitizerTagsArray =
        SanitizerChecker.findTagAndMatchedSanitizer(
          node,
          fclos,
          args,
          null,
      ...
      if (ndResultWithMatchedSanitizerTagsArray) {
        for (const ndResultWithMatchedSanitizerTags of ndResultWithMatchedSanitizerTagsArray) {
          ...
          if (
            !TaintOutputStrategy.isNewFinding(
              this.resultManager,
              taintFlowFinding
            )
          )
          ...
          this.resultManager.newFinding(
            taintFlowFinding,
            TaintOutputStrategy.outputStrategyId
          );
        }
        return true;
      }
  ...
  loadPythonDefaultRule() {
    ...
    try {
      const rulePath = FileUtil.getAbsolutePath(
        "./resource/python/python-default-rule.json"
      );
